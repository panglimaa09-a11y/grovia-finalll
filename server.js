...
app.set('trust proxy',1); app.use(helmet({contentSecurityPolicy:false,crossOriginResourcePolicy:{policy:'cross-origin'}})); app.use(cors({origin:origin||true,credentials:false,methods:['GET','POST','PATCH','DELETE','OPTIONS'],allowedHeaders:['Content-Type','Authorization','X-Youtube-Upload-Url','Content-Range']})); app.use(morgan(process.env.NODE_ENV==='production'?'combined':'dev')); app.use('/api/webhooks',express.raw({type:'application/json',limit:'1mb'}));
app.use('/api/publisher/youtube/upload-chunk',express.raw({type:['application/octet-stream','video/*'],limit:'10mb',inflate:true}));
app.use(express.json({limit:'1mb'})); app.use('/api/',rateLimit({windowMs:60000,limit:120,standardHeaders:true,legacyHeaders:false}));
