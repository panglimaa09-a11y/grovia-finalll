(function(){
  ...
  function lineChart(rows,key,label){
    ...
    const labels=rows.filter((_,i)=>i===0||i===rows.length-1||i===Math.floor(rows.length/2)).map((r)=>{const i=rows.indexOf(r);const x=p+(rows.length===1?gw/2:gw*(i/(rows.length-1)));return `<text x="${x}" y="${h-8}" text-anchor="middle" font-size="10" fill="currentColor" opacity=".55">${esc(r.metric_date)}</text>`}).join('');
  ...
  async function load(){
    ...
    try{
      ...
      if(k)k.innerHTML=`<div class="metric"><small>VIEWS 30 HARI</small><strong>${fmt(s.views)}</strong></div><div class="metric"><small>SUBSCRIBERS GAINED</small><strong>${fmt(rows.reduce((a,x)=>a+Number(x.subscribers_gained||0),0))}</strong></div><div class="metric"><small>WATCH TIME</small><strong>${Number(s.watch_time_minutes||0).toLocaleString('id-ID',{maximumFractionDigits:1})} min</strong></div>`;
      lineChart(rows.map(r=>({metric_date:r.metric_date,views:r.views})), 'views','Views per Day');
      lineChart(rows.map(r=>({metric_date:r.metric_date,subscribers_gained:r.subscribers_gained})), 'subs','Subscribers Gained');
