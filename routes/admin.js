...
r.get('/dashboard', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    res.json(apiOk({ role: req.adminRole, totalUsers: userCount || 0, totalAdminRoles: adminCount || 0, subscriptions: subs.data || [], publishing: jobs.data || [], permissions: ROLE_PERMISSIONS[req.adminRole] || [] }));
  } catch (e) {
...
r.get('/users', requireAdmin, async (req, res) => {
  try {
    const db = req.serviceClient;
    const { data: roles, error: roleError } = await db.from('grovia_admin_roles').select('user_id,role,active,created_at').order('created_at', { ascending: false });
    ...
    if (authAdmin) {
      ...
      else console.warn('ADMIN_AUTH_USERS_READ_FAILED', error.message);
    }
    const ids = new Set([...(roles || []).map(x => x.user_id), ...authUsers.map(x => x.id)]);
    const users = [];
    for (const userId of ids) {
      const auth = authUsers.find(x => x.id === userId) || null;
      ...
      users.push({ user_id: userId, email: auth?.email || '—', last_sign_in_at: auth?.last_sign_in_at || null, created_at: auth?.created_at || profile?.created_at || role.created_at, display_name: profile?.display_name || auth?.user_metadata?.full_name || auth?.user_metadata?.name || '—', role: role.role, active: role.active !== false, subscription: sub || null, social_accounts: social || [], content_count: (content || []).length });
    }
